<x-layouts.admin>
    <x-slot name="title">Customer Groups | Hardware POS</x-slot>
    <x-slot name="pageTitle">Customer Groups</x-slot>

    <div class="space-y-6">
        <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    <h3 class="text-lg font-bold text-slate-900">Customer Group Master</h3>
                    <p class="mt-1 text-sm text-slate-500">Manage customer groups for customer classification.</p>
                </div>
            ...
            <form method="GET" action="{{ route('admin.customer-groups.index') }}" class="mt-6">
                <div class="flex flex-col gap-3 md:flex-row">
                    <input
                        type="text"
                        name="search"
                        value="{{ $search }}"
                        placeholder="Search customer group name or code..."
                ...
                <table class="min-w-full divide-y divide-slate-200">
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        ...
                        <tr class="hover:bg-slate-50">
                            <td class="px-6 py-4 font-semibold text-slate-900">{{ $customerGroup->name }}</td>
                            <td class="px-6 py-4 text-sm text-slate-700">{{ $customerGroup->code }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.customer-groups.edit', $customerGroup) }}" class="rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">
                                        Edit
                                    </a>

                                    <form method="POST" action="{{ route('admin.customer-groups.destroy', $customerGroup) }}" onsubmit="return confirm('Delete this customer group?')">
                                        @csrf
                                        @method('DELETE')
