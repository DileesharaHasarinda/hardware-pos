...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
    <div>
        <label for="name" class="mb-2 block text-sm font-medium text-slate-700">
            Master Category Name <span class="text-red-500">*</span>
        </label>
        <input
            type="text"
            id="name"
            name="name"
            value="{{ old('name', $masterCategory->name ?? '') }}"
            placeholder="Example: Paint, Electrical, Plumbing"
    ...
    <div>
        <label for="code" class="mb-2 block text-sm font-medium text-slate-700">
            Master Category Code <span class="text-red-500">*</span>
        </label>
        <input
            type="text"
            id="code"
            name="code"
            value="{{ old('code', $masterCategory->code ?? '') }}"
            placeholder="Example: PNT, ELE, PLB"
    ...
    <div class="lg:col-span-2">
        <label class="inline-flex items-center gap-3">
            <input
                ...
                name="is_active"
                value="1"
                {{ old('is_active', $masterCategory->is_active ?? true) ? 'checked' : '' }}
...
<div class="mt-8 flex items-center gap-3">
    ...
    >
        {{ $buttonText ?? 'Save Master Category' }}
    </button>
