<x-layouts.admin>
    <x-slot name="title">List of Sizes | Hardware POS</x-slot>
    <x-slot name="pageTitle">List of Sizes</x-slot>

    <div class="space-y-6">
        <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    <h3 class="text-lg font-bold text-slate-900">Size Master</h3>
                    <p class="mt-1 text-sm text-slate-500">Manage size labels like 200ml, 1L, 1Kg and set a default one.</p>
                </div>
            ...
            <form method="GET" action="{{ route('admin.sizes.index') }}" class="mt-6">
                ...
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Default</th>
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        @forelse($sizes as $size)
                            <tr class="hover:bg-slate-50">
                                <td class="px-6 py-4">
                                    <div class="font-semibold text-slate-900">{{ $size->name }}</div>
                                </td>

                                <td class="px-6 py-4">
                                    @if($size->is_default)
                                        <span class="rounded-full bg-blue-100 px-2.5 py-1 text-xs font-semibold text-blue-700">Default</span>
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <form method="POST" action="{{ route('admin.sizes.destroy', $size) }}" onsubmit="return confirm('Delete this size?')">
                                            @csrf
                                            @method('DELETE')
                                            <button
                                                type="submit"
